...
export default function RouteBox({ destination, awaitingStart, onFindRoute, onUseCurrentLocation, onClear }) {
  return (
        ...
        {destination ? (
          <>
            <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ fontWeight: 700, color: 'var(--text)' }}>{destination.name || `${destination.lat.toFixed(5)}, ${destination.lng.toFixed(5)}`}</div>
              <div style={{ fontSize: 12, color: 'var(--muted)' }}>{destination.lat.toFixed(5)}, {destination.lng.toFixed(5)}</div>
            </div>
